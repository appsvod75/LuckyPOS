...
const POS: React.FC = () => {
  const { cart, addToCart, removeFromCart, updateQuantity, updateItemVariant, updateItemNotes, subtotal, total, clearCart } = useCart();
  ...
  const handleShowMedicalInfo = async (e: React.MouseEvent, product: any) => {
    e.stopPropagation();
    setSelectedProduct(product);
    setLoadingAi(true);
    setMedicalInfo(null);
    try {
      const res = await productApi.getMedicalInfo(product.id);
    ...
    const handleConfirmCheckout = async (paymentMethod: string, amountTendered: number, targetClient: any | null, dueDate?: string, customDate?: string) => {
    ...
    <div className={`pos-container ${isSidebarOpen ? 'sidebar-expanded' : ''}`}>
            ...
            {filteredProducts.map((p) => (
              ...
              >
                <div className="product-image-container">
                  ...
                  {p.is_medicine && (
                    <button className="ai-info-btn" onClick={(e) => handleShowMedicalInfo(e, p)}>
                      <Info size={16} />
                    </button>
                  )}
                </div>
      ...
      {selectedProduct && (
          <div className="modal-overlay" onClick={() => setSelectedProduct(null)}>
            ...
            >
              <div className="modal-header" style={{ background: '#0f172a', color: 'white', borderBottom: 'none' }}>
                <div>
                  <h2 style={{ color: 'white', margin: 0 }}>{selectedProduct.name}</h2>
                  <p style={{ color: '#94a3b8', fontSize: '0.8rem', margin: '4px 0 0' }}>Información Médica detallada</p>
                </div>
                <button onClick={() => setSelectedProduct(null)} style={{ color: 'white', opacity: 0.7 }}><X /></button>
              </div>
              <div className="modal-body">
                {loadingAi ? (
                  <div className="ai-loading" style={{ color: '#0f172a' }}>Generando información con Gemini AI...</div>
                ) : medicalInfo ? (
                  <div className="medical-info-grid">
                    <div className="info-card">
                      <label>¿Para qué sirve?</label>
                      <p>{medicalInfo.paraQueSirve}</p>
                    </div>
                    <div className="info-card">
                      <label>Principio Activo</label>
                      <p>{medicalInfo.principioActivo}</p>
                    </div>
                    <div className="info-card">
                      <label>Dosis Adulto</label>
                      <p>{medicalInfo.dosisAdulto}</p>
                    </div>
                    <div className="info-card">
                      <label>Dosis Niño</label>
                      <p>{medicalInfo.dosisNino}</p>
                    </div>
                    <div className="info-card danger">
                      <label>Contraindicaciones</label>
                      <p>{medicalInfo.contraindicaciones}</p>
                    </div>
                    <div className="info-card danger">
                      <label>Efectos Secundarios</label>
                      <p>{medicalInfo.efectosSecundarios}</p>
                    </div>
                  </div>
                ) : <p>Error al obtener información.</p>}
              </div>
            </motion.div>
          </div>
        )}
      ...
      <style>{`
        ...
        .ai-info-btn {
          ...
          border-radius: 50%;
          cursor: pointer;
          box-shadow: 0 4px 10px ${user.color_hex || '#3b82f6'}60;
        }
        .product-stock.low {
          color: #ef4444;
          font-weight: 700;
        ...
        .medical-info-grid { display: grid; grid-template-columns: repeat(2, 1fr); gap: 1rem; }
        .info-card { background: #f8fafc; padding: 1rem; border-radius: 16px; border: 1px solid #e2e8f0; }
        .info-card label { display: block; font-size: 0.7rem; font-weight: 800; text-transform: uppercase; color: #64748b; margin-bottom: 0.5rem; }
        .info-card p { font-size: 0.9rem; line-height: 1.5; color: #1e293b; }
        .info-card.danger { border-color: #fee2e2; background: #fffafb; }
        .info-card.danger label { color: #b91c1c; }
        .ai-loading { text-align: center; padding: 3rem; color: #64748b; font-weight: 500; }
