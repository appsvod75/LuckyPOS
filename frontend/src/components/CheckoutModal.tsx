...
import { Search, MapPin, Calendar, CreditCard, Banknote, UserPlus, X, ShoppingCart, Plus, CheckCircle2, ChevronRight, Save, Building2, Delete, User, Phone } from 'lucide-react';
...
interface CheckoutModalProps {
    orderTotal: number;
    onClose: () => void;
    onConfirm: (paymentMethod: string, amountTendered: number, targetClient: any | null, dueDate?: string, customDate?: string) => void;
}
...
const CheckoutModal: React.FC<CheckoutModalProps> = ({ orderTotal, onClose, onConfirm }) => {
    ...
    const canConfirm = useMemo(() => {
        ...
        return true;
    }, [amountTendered, orderTotal, method]);
    ...
    const confirmSale = () => {
        ...
        onConfirm(method, finalTendered, selectedClient, method === PaymentMethod.CREDIT ? dueDate : undefined, isAdmin ? saleDate : undefined);
    };
            ...
            <span>
                {parts.map((part, i) => 
                    part.toLowerCase() === query.toLowerCase() 
                        ? <strong key={i} style={{ color: '#10b981', fontWeight: 900 }}>{part}</strong>
    ...
    return (
        <div className={`checkout-overlay ${isClientSearchFocused ? 'search-active' : ''}`} onClick={onClose}>
                        ...
                        >
                            ...
                            <div className="new-client-footer">
                                ...
                                <button
                                    ...
                                    className={`nc-btn-save ${!newClientData.name ? 'disabled' : ''}`}
                                >
                                    Guardar Cliente
                                </button>
                            </div>
                ...
                <div className="checkout-modal">
                    ...
                    <div className="checkout-content">
                        ...
                        <div className="section-card">
                            <h4 className="section-title"><Search size={14} /> CLIENTE (OPCIONAL)</h4>
                            {selectedClient ? (
                                            ...
                                            <div className="client-results-dropdown">
                                                {filteredClients.length > 0 ? (
                                                    filteredClients.map(c => (
                                                        <button key={c.id} onClick={() => { setSelectedClient(c); setSearchClient(''); }} className="result-item">
                        ...
                        {isAdmin && (
                            <div className="section-card">
                                <h4 className="section-title"><Calendar size={14} /> FECHA DE FACTURACIÓN <span className="text-[0.6rem] ml-1 opacity-70">(ADMIN)</span></h4>
                        ...
                        <div className="section-card amount-card">
                            <h4 className="section-title"><Banknote size={14} /> MONTO RECIBIDO</h4>
                            <div className="amount-input-wrapper">
                                <span className="currency-symbol">$</span>
                                <input 
                                    type="text" 
                                    value={method === PaymentMethod.CASH ? amountTendered : orderTotal.toFixed(2)}
                                    onChange={e => method === PaymentMethod.CASH && setAmountTendered(e.target.value.replace(/[^0-9.]/g, ''))}
                                    placeholder={orderTotal.toFixed(2)}
                                    className="checkout-amount-input"
                                    onFocus={() => method === PaymentMethod.CASH && setIsAmountKeyboardOpen(true)}
                                    inputMode="none"
                                />
                            </div>
                            {method === PaymentMethod.CASH && changeDue > 0 && (
            ...
            <style>{`
                ...
                .checkout-modal {
                    background: #0f172a; width: 100%; max-width: 420px;
